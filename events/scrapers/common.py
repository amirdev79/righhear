...
ARTIST_FIELDS = ['id', 'first_name', 'last_name', 'first_name_heb', 'last_name_heb', 'image', 'category',
                 'sub_categories', 'media1 id', 'media1 type', 'media1 link', 'media1 playback start',
                 'media1 playback end', 'media2 id', 'media2 type', 'media2 link', 'media2 playback start',
                 'media2 playback end', 'media3 id', 'media3 type', 'media3 link', 'media3 playback start',
                 'media3 playback end']
...
def _update_existing_artist_media(artist, media_fields):
    if len(media_fields) == 0:
        ...
        artist.media.clear()
        if len(media_fields) >= 5:
            media1_id, media1_type, media1_link, media1_playback_start, media1_playback_end = media_fields[:5]
            if media1_id != '':
                artist.media.add(int(media1_id))
        if len(media_fields) >= 10:
            media2_id, media2_type, media2_link, media2_playback_start, media2_playback_end =   media_fields[5:10]
            if media2_id != '':
                artist.media.add(int(media2_id))
        if len(media_fields) >= 15:
            media3_id, media3_type, media3_link, media3_playback_start, media3_playback_end = media_fields[10:15]
            if media3_id != '':
                artist.media.add(int(media3_id))
